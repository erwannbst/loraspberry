...
from .board_config import SERVER_BOARD, CLIENT_BOARD
...
class LoRa(object):
    ...
    spi = SERVER_BOARD.SpiDev()              # init and get the baord's SPI
    ...
    def __init__(self, verbose=True, do_calibration=True, calibration_freq=868):
        ...
        SERVER_BOARD.add_events(self._dio0, self._dio1, self._dio2, self._dio3, self._dio4, self._dio5)
    ...
    def get_pkt_rssi_value(self):
        ...
        return v - (164 if SERVER_BOARD.low_band else 157)     # See datasheet 5.5.5. p. 87
...
class LoRa2(object):
    ...
    spi = CLIENT_BOARD.SpiDev()              # init and get the baord's SPI
    mode = None                       # the mode is backed up here
    backup_registers = []
    verbose = True
    dio_mapping = [None] * 6          # store the dio mapping here
    ...
    def __init__(self, verbose=True, do_calibration=False, calibration_freq=868):
        ...
        self.verbose = verbose
        # set the callbacks for DIO0..5 IRQs.
        CLIENT_BOARD.add_events(self._dio0, self._dio1, self._dio2, self._dio3, self._dio4, self._dio5)
        # set mode to sleep and read all registers
        self.set_mode(MODE.SLEEP)
        self.backup_registers = self.get_all_registers()
        # more setup work:
    ...
    def get_pkt_rssi_value(self):
        v = self.spi.xfer([REG.LORA.PKT_RSSI_VALUE, 0])[1]
        return v - (164 if CLIENT_BOARD.low_band else 157)     # See datasheet 5.5.5. p. 87
